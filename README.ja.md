...
**ツンデレ**も**心理的安全性**も仕組みは同じ：**トグルスイッチ**でスキンをONにし、**中央OFFの両極スライダー**で口調／側を決めます。メニューバーでは速さ/高さ/抑揚の下にトグル＋スライダーで並び、⚙設定ウィンドウにもあります。各ペインのサブメニューで**全パラメータ**（名前・声・音量・ツンデレ・心理的安全性・速さ/高さ/抑揚）を個別に上書きできます。両方ONなら**組み合わさります**：心理的安全性の**側（ブラック/ホワイト）が環境**、ツンデレの**口調（ツン/デレ）が人格**を決め、`ブラック×デレ`（「こんな環境なのにえらいよ、でも体だけは大事にしてね？」）と `ブラック×ツン`（冷たく詰める）は別物になります。片方OFFならもう片方だけが効きます。
