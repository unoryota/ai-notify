...
ai-notify menubar install  # macOS: メニューバーの 🔔 を追加（任意・推奨）
```

`init` が Claude Code / Codex / Gemini を見つけてフックを配線します。これだけで音・読み上げ・デスクトップ通知は動きます。**メニューバーのアイコンは別ステップ**です：`ai-notify menubar install`（macOS のみ）を実行すると、左クリックで音量/声、右クリックでミュートできる 🔔 が出ます。あとは1つのスイッチで全部を操作できます：
...
```sh
ai-notify init [--dry-run] [--only claude,codex,gemini]   # 検出したエージェントを配線
ai-notify use <名前> [声] [音量] [--tab <名前>]      # このペインの名前＋声＋タブ名を一括設定
ai-notify toggle | on | off | status               # ミュートスイッチ
ai-notify volume [0.0-2.0]                          # 音量の取得／設定
ai-notify summary [0.0-1.0]                         # 要約度: 0=効果音のみ … 0.5=約10秒 … 1=全文読み上げ
ai-notify voice [number|name|preview|default]      # 読み上げ音声を選ぶ
ai-notify voicevox [setup|on <id>|off|speakers|test] # VOICEVOXの声で読み上げ
...
> **`init` してもアイコンが出ない？** `init` はエージェントのフックを配線するだけで、メニューバーアプリは上の**別コマンド**で入れます。それでも 🔔 が出ないときは：まず**macOS 専用**です。ビルドに Swift ツールチェーンが必要なので、`ai-notify menubar status` が `built: —` なら `xcode-select --install`（Command Line Tools）を実行。`brew upgrade` 後はインストール先パス（バージョン付き）が変わるため、`ai-notify menubar install` を再実行してください。
...
- **左クリック** → メニュー：**音量**スライダー・読み上げ（**速さ/高さ/抑揚**）スライダー・**ツンデレ**と**心理的安全性**の各**ON/OFFトグルスイッチ＋両極スライダー**（どちらも中央＝OFF）、**声の一覧**（システム＋VOICEVOX）、そして各ターミナルの**ペイン別サブメニュー**。ペイン別では**全パラメータ**（読み上げ名・声・音量・ツンデレ・心理的安全性・速さ/高さ/抑揚）を個別に上書きできます。ペイン一覧には**ライブな**エージェントが動いているターミナルだけが出ます — 閉じたセッションや再起動で失われたペインは**自動で掃除**され（メニューバー起動時＋メニューを開くたび）、古い「ゴースト」ペインが残りません。
- **右クリック** → 即ミュート切替。
...
キーレス・無料（同じ文言ごとに HTTP 1リクエスト。2回目以降はキャッシュから即時返すのでオフラインでも安全。翻訳を取得できないときはローカルの定型文にフォールバック）。デスクトップ通知には原文も表示されます。
