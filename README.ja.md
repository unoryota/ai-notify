...
ai-notify war [level <0-1>|test|status]            # アドレナリン／戦争モード（スライダー・左端0=平時OFF→右で戦争）
...
- **左クリック** → メニュー：**音量**・読み上げ（**速さ/高さ/抑揚**）・**ツンデレ**・**アドレナリン**の青スライダー（ツンデレ＝中央でOFF・アドレナリン＝左端でOFF）、**声の一覧**（システム＋VOICEVOX）、そして各ターミナルの**ペイン別サブメニュー**。ペイン別では**全パラメータ**（読み上げ名・声・音量・ツンデレ・アドレナリン・速さ/高さ/抑揚）を個別に上書きできます。
- **右クリック** → 即ミュート切替。
- **⚙ 設定…** → **整列したスライダー＋編集可能な数値フィールド**と**プリセット保存**（`ai-notify preset save <名前>` / `load` / `delete`）の設定ウィンドウ。毎回調整し直さなくて済みます。

<img alt="ai-notify メニュー — 音量・速さ/高さ/抑揚・ツンデレ・アドレナリンの青スライダー、声の一覧、ターミナルごとのペイン別行" src="https://raw.githubusercontent.com/unoryota/ai-notify/main/assets/menubar.png" width="280">

*メニュー：青スライダー（音量・速さ/高さ/抑揚・ツンデレ・アドレナリン）、声の一覧、そしてターミナルごとのペイン別行（🗣 名前 — 🔊 声）。各行を開くとそのペイン専用の上書き設定が出ます。*
...
読み上げに「ツンデレ」人格を載せます。スライダーは**中央(0.5)＝OFFの両極**。**左に振るほどツン**（最左端は隠れデレ無しの**極寒デレ0**「…で、それが何か？」）、**右に振るほどデレ**（最右端は甘々の**デレデレ**）。**5段階**に刻んであります：極寒 ← ツン ←（OFF）→ デレ → デレデレ。
...
```sh
ai-notify war level 0.85     # 0=OFF/平時（左端） 〜 1=危機 の強度スライダー
ai-notify war on/off         # 便利コマンド: アクティブ(0.85) / OFF(最小0・平時) へ
ai-notify war test
```

- **平時** — 落ち着いた無線。**戦闘中** — 第一種戦闘配置、緊迫。**危機** — 短い絶叫、音量↑・速度↑。
- **ツンデレレベルが各段を味付け**（デレ＝優しい／ツン＝厳しいオペレーター）。戦争×ツンデレで9通り。
- どちらも**スライダーのみ**（ON/OFFチェックなし）。**ツンデレ**＝中央でOFF（**左ツン/右デレ**・最左端=極寒／最右端=デレデレ）。**アドレナリン**＝**左端（最小）＝平時**→右で戦争/危機、の強度スライダー。メニューバーでは速さ/高さ/抑揚の下に青スライダーで、⚙設定ウィンドウにもあります。各ペインのサブメニューで**全パラメータ**（名前・声・音量・ツンデレ・アドレナリン・速さ/高さ/抑揚）を個別に上書きできます。
