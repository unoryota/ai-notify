...
読み上げに「ツンデレ」人格を載せます。スライダーは**中央(0.5)＝OFFの両極**。**左に振るほどデレ**（最左端は甘々の**デレデレ**）、**右に振るほどツン**（最右端は隠れデレ無しの**極寒デレ0**「…で、それが何か？」）。**5段階**に刻んであります：デレデレ ← デレ ←（OFF）→ ツン → 極寒。

```sh
ai-notify tsundere level 0.7     # 0=デレ … 0.5=OFF … 1=ツン。メニューバーにもスライダー
...
- どちらも**スライダーのみ**（ON/OFFチェックなし）。**ツンデレ**＝中央でOFF（左デレ/右ツン）。**アドレナリン**＝**左端（最小）でOFF**→右で危機、の強度スライダー。メニューバーでは速さ/高さ/抑揚の下に青スライダーで、⚙設定ウィンドウにもあります。各ペインのサブメニューで**全パラメータ**（名前・声・音量・ツンデレ・アドレナリン・速さ/高さ/抑揚）を個別に上書きできます。
